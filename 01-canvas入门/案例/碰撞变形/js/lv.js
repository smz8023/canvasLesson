...
class Geo{
    ...
    /*位移*/
    //先根据极坐标获取几个点位，再将其添加到当前点位
    translateOnAxis(axis,distance){
        this.vertices.forEach((ele)=>{
            ele.pos.add(Vector2.polar(distance,axis.angle()));
        })
    }
    transLate(x,y){
        this.vertices.forEach((ele)=>{
...
//矩形几何体
class RectGeo extends Geo{
    constructor(w=0,h=0,baseCenter=false){
        super();
    ...
    update(){
        const {width,height,baseCenter}=this;
        if(baseCenter){
...
class Poly extends Sprite{
    ...
    draw(ctx){
        ctx.beginPath();
        this.geo.vertices.forEach((ele,ind)=>{
            if (ind===0){
